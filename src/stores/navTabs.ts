...
export const useNavTabs = defineStore(
    ...
    () => {
        ...
        function _addTab(route: RouteLocationNormalized) {
            ...
            const tabViewId = tabView.meta.id
            const hasValidTabViewId = tabViewId !== undefined && tabViewId !== null && String(tabViewId) !== ''

            for (const currentTab of state.tabsView) {
                // 菜单已在 tabs 存在，更新 params 和 query
                const currentTabId = currentTab.meta.id
                const hasSameValidId = hasValidTabViewId && currentTabId !== undefined && currentTabId !== null && String(currentTabId) !== '' && currentTabId === tabViewId
                if (hasSameValidId || currentTab.fullPath == tabView.fullPath) {
                    currentTab.fullPath = tabView.fullPath
                    currentTab.params = !isEmpty(tabView.params) ? tabView.params : currentTab.params
                    currentTab.query = !isEmpty(tabView.query) ? tabView.query : currentTab.query
                    return
                }
            }
        ...
        const getTabsViewDataByPath = (path: string, menus: RouteRecordRaw[], returnType: 'normal' | 'above'): RouteRecordRaw | false => {
            for (const menu of menus) {
                ...
                if (menu.children && menu.children.length) {
                    const find = getTabsViewDataByPath(path, menu.children, returnType)
                    if (find) {
                        return returnType == 'above' ? menu : find
                    }
                }
            }
            return false
